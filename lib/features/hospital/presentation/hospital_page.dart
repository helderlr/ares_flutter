...
import '../services/hospital_service_paginado.dart';
import '../models/hospital_model.dart';
import 'consulta_hospital_page.dart';
...
class _HospitalPageState extends State<HospitalPage> {
  final HospitalServicePaginado _service = HospitalServicePaginado();
  final List<Hospital> _allHospitais = [];
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _searchController = TextEditingController();

  HospitalPaginationInfo? _pagination;
  ...
  void initState() {
    super.initState();
    print('🚀 Inicializando HospitalPage');
  ...
  Future<void> _loadFirstPage() async {
    ...
    try {
      ...
      final response = await _service.fetchHospitaisPaginated(
        page: 1,
        pageSize: 50,
      ...
      setState(() {
        _allHospitais.addAll(response.hospitais);
      ...
      print(
          '✅ Primeira página carregada: ${response.hospitais.length} hospitais');
  ...
  Future<void> _loadNextPage() async {
    ...
    try {
      ...
      print(
          '✅ Próxima página carregada: ${response.hospitais.length} hospitais');
    } catch (e) {
  ...
  void _handleError(dynamic error) {
    ...
    if (error.toString().contains('404') ||
        error.toString().contains('Nenhum hospital encontrado')) {
      return;
    }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Lista de hospitais
          Expanded(
            child: _isLoading && _allHospitais.isEmpty
                ? const Center(child: CircularProgressIndicator())
                : _allHospitais.isEmpty
                    ? Center(
                        child: Column(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: ListView.separated(
                              ...
                              itemCount: _allHospitais.length,
                              ...
                              itemBuilder: (context, index) {
                                final hospital = _allHospitais[index];
            ...
            IconButton(
              ...
              onPressed: () async {
                ...
                final result = await Navigator.of(context).push<bool>(
                  MaterialPageRoute(
                    builder: (context) => const HospitalFormPage(),
                  ),
                );

                // Se criou um novo hospital, recarrega a lista
                if (result == true) {
  ...
  Widget _buildHospitalItem(Hospital hospital) {
    return InkWell(
      onTap: () async {
        ...
        final result = await Navigator.of(context).push<bool>(
          MaterialPageRoute(
            builder: (context) => ConsultaHospitalPage(hospital: hospital),
          ),
        );
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  if (hospital.fantasyName != 'Nome fantasia não disponível')
                    Text(
                      'Nome Fantasia: ${hospital.fantasyName}',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
